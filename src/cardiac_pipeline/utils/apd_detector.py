...
DEFAULT_MIN_AMP_ABS = 100.0      # absolute floor (ADC counts)
...
def detect_all_apd_levels_pixel(
    ...
    VSD inverted polarity:
      - baseline ≈ 0 (rest state)
      - apex = MIN value (most negative during upstroke)
    ...
    #      local AP: baseline (most positive), apex (most negative), upstroke.
    ...
    # Local baseline = most positive value (VSD inverted at rest)
    local_baseline = float(np.max(full_seg))

    # Local apex = most negative value (deepest point of AP)
    ...
    # Find onset: first frame AFTER previous apex where signal drops below
    # 20% threshold (start of THIS upstroke)
    # For VSD inverted: signal goes from baseline (positive) DOWN to apex (negative)
    # We need the frame where signal crosses 20% down from baseline
    ...
    for level in levels:
        ...
        # Repolarization target (between apex and baseline)
        #    For VSD inverted: signal must rise BACK from apex
        repol_target = apex_val + level_frac * (baseline - apex_val)

        # Find first frame after apex where signal > repol_target
